...
bool kvengine_send_put_rts_req(TupleKeySlice key, TupleValueSlice value, RangeID rangeid)
{
	Dataslice req_key = (Dataslice)palloc0(key.len + sizeof(Size));
	Dataslice req_value = (Dataslice)palloc0(value.len + sizeof(Size));
	make_data_slice(*req_key, key);
	...
	save_slice_into_buffer((char *)req->k_v + index, req_value);
	range_free(req_value);
	req->header.type = ROCKSDB_PUTRTS;
	req->header.size = size;
	req->rangeid = rangeid;
	/*
	 * The response just tell session that the request has been processed.
	 * It contains no informations. So we don't need return it.
	 */
	send_kv_request((RequestHeader *)req);
	range_free(req);
	return true;
...
ScanResponse *
kvengine_send_index_scan_req(TupleKeySlice startkey, TupleKeySlice endkey,
							 ...
							 CmdType Scantype)
{
	...
	Size size = sizeof(ScanWithKeyRequest) + startkey.len +
				sizeof(Size) + endkey.len + sizeof(Size);
...
void second_and_search_again(KVEngineScanDesc scan, TupleTableSlot *slot)
{
	...
	if (pk_Oid != scan->index_id)
	{
		...
		for (int i = 0; i < scan->scan_num; i++)
		{
			...
			{
				// SSS
				if (consistency_mode == CONSISTENCY_MODE_DEFAULT &&
					transam_mode == TRANSAM_MODE_NEW_OCC)
				{
					temp = decode_pkey_raw(scan->cur_key[i], scan->rks_rel, pk_Oid);
				}
				// SS
				else if (consistency_mode == CONSISTENCY_MODE_SEQUENCE &&
						 transam_mode == TRANSAM_MODE_NEW_OCC)
				{
					temp = decode_pkey_lts(scan->cur_key[i], scan->rks_rel, pk_Oid);
				}
				// CAUSAL
				...
				// PURE RUCC
				else if (consistency_mode == CONSISTENCY_MODE_RUCC &&
						 transam_mode == TRANSAM_MODE_NEW_OCC)
				{
					temp = decode_pkey_lts(scan->cur_key[i], scan->rks_rel, pk_Oid);
				}
...
{
	ScanResponse *scan_res = kvengine_send_index_scan_req(scan->next_key, scan->end_key,
...
void pick_to_slot(KVEngineScanDesc scan, TupleTableSlot *slot)
{
	...
	TupleValueSlice newvalue = {NULL, 0};

	if (transam_mode == TRANSAM_MODE_NEW_OCC &&
		(consistency_mode == CONSISTENCY_MODE_SEQUENCE || 
		 consistency_mode == CONSISTENCY_MODE_RUCC))
	{
		bool result = DTAScanCallBack(key.len, (char *)key.data, &newkey, &newvalue);
		if (result)
		{
			key = newkey;
			value = newvalue;
			range_free(scan->cur_key[scan->scan_index].data);
			range_free(scan->cur_value[scan->scan_index].data);
			scan->cur_key[scan->scan_index] = newkey;
			scan->cur_value[scan->scan_index] = newvalue;
		}
	}
...
bool kvengine_commit(void)
{
	...
	if (res && cres->success)
	{
		...
		if (consistency_mode == CONSISTENCY_MODE_SEQUENCE)
			SequenceGTSUpdate(global_tmp_timestamp.commit_ts);
		return true;
	}
	else
	{
