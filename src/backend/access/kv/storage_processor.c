...
void get_key_interval_from_scan_req(ScanWithKeyRequest *scan_req, TupleKeySlice *start_key, TupleKeySlice *end_key)
{
...
kvengine_process_get_req(RequestHeader *req)
{
	...
	{
		// SSS
		if (req->con_mode == CONSISTENCY_MODE_DEFAULT &&
			req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			return (ResponseHeader *)kvengine_optprocess_get_req(req);
		}
		// SS
		else if (req->con_mode == CONSISTENCY_MODE_SEQUENCE &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			return (ResponseHeader *)kvengine_ltsprocess_get_req(req);
		}
		// CAUSAL
		else if (req->con_mode == CONSISTENCY_MODE_CAUSAL)
		{
			return (ResponseHeader *)kvengine_optprocess_get_req(req);
		}
		// pure rucc
		else if (req->con_mode == CONSISTENCY_MODE_RUCC &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			return (ResponseHeader *)kvengine_ltsprocess_get_req(req);
		}
...
kvengine_process_put_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		if (req->con_mode == CONSISTENCY_MODE_DEFAULT &&
			req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			kvengine_optprocess_put_req(req);
			return (ResponseHeader *)kvengine_ltsprocess_put_req(req);
		}
		// SS
		else if (req->con_mode == CONSISTENCY_MODE_SEQUENCE &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
...
kvengine_process_delete_normal_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		if (req->con_mode == CONSISTENCY_MODE_DEFAULT &&
			req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			kvengine_optprocess_delete_normal_req(req);
			return (ResponseHeader *)kvengine_ltsprocess_delete_normal_req(req);
		}
		// SS
		else if (req->con_mode == CONSISTENCY_MODE_SEQUENCE &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
...
kvengine_process_update_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		if (req->con_mode == CONSISTENCY_MODE_DEFAULT &&
			req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			kvengine_optprocess_update_req(req);
			return (ResponseHeader *)kvengine_ltsprocess_update_req(req);
		}
		// SS
		else if (req->con_mode == CONSISTENCY_MODE_SEQUENCE &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
...
kvengine_process_delete_direct_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		if (req->con_mode == CONSISTENCY_MODE_DEFAULT &&
			req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			kvengine_optprocess_delete_direct_req(req);
			return (ResponseHeader *)kvengine_ltsprocess_delete_direct_req(req);
		}
		// SS
		else if (req->con_mode == CONSISTENCY_MODE_SEQUENCE &&
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
...
kvengine_process_scan_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		// SS
				 ...
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			return (ResponseHeader *)kvengine_ltsprocess_scan_req(req);
		}
		// CAUSAL
		else if (req->con_mode == CONSISTENCY_MODE_CAUSAL)
		{
...
kvengine_process_multi_get_req(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		// SS
				 ...
				 req->txn_mode == TRANSAM_MODE_NEW_OCC)
		{
			return (ResponseHeader *)kvengine_ltsprocess_multi_get_req(req);
		}
		// CAUSAL
		else if (req->con_mode == CONSISTENCY_MODE_CAUSAL)
		{
...
kvengine_process_commit(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		case TRANSAM_MODE_NEW_OCC:
			return (ResponseHeader *)kvengine_ltsprocess_commit(req);
		default:
...
kvengine_process_abort(RequestHeader *req)
{
	...
	case KVENGINE_TRANSACTIONDB:
	{
		...
		{
		case TRANSAM_MODE_NEW_OCC:
			return (ResponseHeader *)kvengine_ltsprocess_abort(req);
		default:
