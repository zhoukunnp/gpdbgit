...
DtxTimeStamp global_tmp_timestamp = {0, 0, 0, (uint32) -1};
...
{
	int	size = 0;
	size += sizeof(uint64) + sizeof(uint64) + sizeof(int) + sizeof(int);
	return size;
}
...
DtxTimeStamp_Serialize(char *buffer, DtxTimeStamp *dtxts)
{
	...
	memcpy(p, &dtxts->lts_low, sizeof(int));
	p += sizeof(int);

	memcpy(p, &dtxts->lts_high, sizeof(int));
	p += sizeof(int);
}
									   ...
									   DtxTimeStamp *dtxts)
{
	...
	if (serializeddtxtslen > 0)
	{
		...
		p += sizeof(uint64);
		memcpy(&dtxts->lts_low, p, sizeof(int));
		p += sizeof(int);
		memcpy(&dtxts->lts_high, p, sizeof(int));
		p += sizeof(int);
	}
}
