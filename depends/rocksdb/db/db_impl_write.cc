...
//   return WriteImpl(write_options, my_batch, callback, nullptr);
  return ConcurrentWriteImpl(write_options, my_batch, callback, nullptr);
//   return RTSWriteImpl(write_options, my_batch, callback, nullptr);
}
#endif  // ROCKSDB_LITE

// write log and memtable independence without group commit
Status DBImpl::ConcurrentWriteImpl(const WriteOptions& write_options,
                                   ...
                                   PreReleaseCallback *pre_release_callback)
{
  ...
  if (my_batch == nullptr) {
    ...
    const int64_t commit_ts = my_batch->GetCommitTS();
    WriteBatchInternal::SetSequence(my_batch, commit_ts);
    WriteThread::Writer w(write_options, my_batch, callback, log_ref, disable_memtable, batch_cnt,
        pre_release_callback);

    // const uint64_t last_sequence = versions_->LastSequence();
    ...
    //                         need_log_dir_sync, commit_ts);
      w.status = WriteBatchInternal::InsertInto(&w, commit_ts, &column_family_memtables,
          &flush_scheduler_, write_options.ignore_missing_column_families,
          0, this, true, false, w.batch_cnt);

      has_unpersisted_data_.store(true, std::memory_order_relaxed);
      versions_->SetLastSequenceIfLarge(WriteBatchInternal::Sequence(my_batch));
    } else {
      //TRANS_LOG_ERROR("check CheckKeysForConflicts failed");
    }

    /*TRANS_LOG_DEBUG("concurrent_writeimpl commit_ts:%lu last_sequence:%lu version sequece:%lu "
        "need_log_sync:%d disable_memtable:%d writebatch:%s batch_cnt:%lu status:%s w:%s",
        commit_ts, last_sequence, versions_->GetLastSequence(), need_log_sync, disable_memtable,
        my_batch->TO_HEX(), w.batch_cnt, status.TO_STR(), w.status.TO_STR());*/
  }
...
//   const int64_t commit_ts = my_batch->GetCommitTS();
  ...
  if (status.ok()) {
    ...
    // const SequenceNumber current_sequence = commit_ts;
...
// When using one write queue, the same sequence also indicates the last
// published sequence.
Status DBImpl::RTSWriteImpl(const WriteOptions& write_options,
                         WriteBatch* my_batch, WriteCallback* callback,
                         uint64_t* log_used, uint64_t log_ref,
                         bool disable_memtable, uint64_t* seq_used,
                         size_t batch_cnt,
  ...
  if (w.state == WriteThread::STATE_PARALLEL_MEMTABLE_WRITER) {
    ...
    if (write_thread_.CompleteParallelMemTableWriter(&w)) {
      ...
      // TODO(myabandeh): propagate status to write_group
      auto last_sequence = w.write_group->last_sequence;
      versions_->SetLastSequenceIfLarge(last_sequence);
      MemTableInsertStatusCheck(w.status);
      write_thread_.ExitAsBatchGroupFollower(&w);
    }
    assert(w.state == WriteThread::STATE_COMPLETED);
  ...
  if (should_exit_batch_group) {
    if (status.ok()) {
      ...
      versions_->SetLastSequenceIfLarge(commit_ts);
    }
    MemTableInsertStatusCheck(w.status);
    write_thread_.ExitAsBatchGroupLeader(write_group, status);
  }
