...
OptimisticTransaction::OptimisticTransaction(
  ...
  OptimisticTransaction::OptimisticTransaction(
      OptimisticTransactionDB* txn_db, const WriteOptions& write_options,
    const OptimisticTransactionOptions& txn_options, const SequenceNumber start_ts)
    : TransactionBaseImpl(txn_db->GetBaseDB(), write_options), txn_db_(txn_db){
    
    Initialize(txn_options, start_ts);
}
...
Status OptimisticTransaction::CommitWithTS(const SequenceNumber commit_ts) {
  // Set up callback which will call CheckTransactionForConflicts() to
  // check whether this transaction is safe to be committed.
  OptimisticTransactionCallback callback(this);

  DBImpl* db_impl = static_cast_with_check<DBImpl, DB>(db_->GetRootDB());
  WriteBatch* my_batch = GetWriteBatch()->GetWriteBatch();
  my_batch->SetCommitTS(commit_ts);
  WriteBatchInternal::SetSequence(my_batch, commit_ts);
  Status s = db_impl->WriteWithCallback(
      write_options_, my_batch, &callback);
...
Status OptimisticTransaction::CommitWithLTS(const SequenceNumber commit_ts, const uint32_t lts) {
  // Set up callback which will call CheckTransactionForConflicts() to
  // check whether this transaction is safe to be committed.
  OptimisticTransactionCallback callback(this);
