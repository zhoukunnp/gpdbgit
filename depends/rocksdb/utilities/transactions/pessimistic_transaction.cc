...
Status PessimisticTransaction::CommitWithTS(const SequenceNumber commit_ts) {
  Status s;
  bool commit_without_prepare = false;
  bool commit_prepared = false;
  ...
  if (commit_without_prepare) {
    ...
    if (WriteBatchInternal::Count(GetCommitTimeWriteBatch()) > 0) {
      ...
      s = CommitWithoutPrepareInternalWithTS(commit_ts, 0);
      if (!name_.empty()) {
        txn_db_impl_->UnregisterTransaction(this);
      }
      Clear();
    ...
    s = CommitInternalWithTS(commit_ts, 0);
...
Status PessimisticTransaction::CommitWithLTS(const SequenceNumber commit_ts, const uint32_t lts) {
  Status s;
  bool commit_without_prepare = false;
  bool commit_prepared = false;
  ...
  if (commit_without_prepare) {
    ...
    if (WriteBatchInternal::Count(GetCommitTimeWriteBatch()) > 0) {
      ...
      s = CommitWithoutPrepareInternalWithTS(commit_ts, lts);
      if (!name_.empty()) {
        txn_db_impl_->UnregisterTransaction(this);
      }
      Clear();
    ...
    s = CommitInternalWithTS(commit_ts, lts);
...
Status WriteCommittedTxn::CommitWithoutPrepareInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  uint64_t seq_used = kMaxSequenceNumber;
...
Status WriteCommittedTxn::CommitInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  // We take the commit-time batch and append the Commit marker.
  // The Memtable will ignore the Commit marker in non-recovery mode
  WriteBatch* working_batch = GetCommitTimeWriteBatch();
  working_batch->SetCommitTS(commit_ts);
