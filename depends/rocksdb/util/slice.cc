...
bool Slice::SliceNeedTransferToWithLTS(const Slice& a) const {
  //char* buf = (char*)malloc(a.size());
//   new char[a.size()];
  //strncpy(buf, a.data(), a.size());
  size_t len = a.size();
  //char* label = buf + len - 1;
  char ll = *(a.data() + len - 1);
  if (ll == 'l')
    return true;
  else
    return false;
...
void Slice::SliceWithLTS(const Slice& a, Slice* b, uint32_t LTS) const {
  size_t len = a.size() + sizeof(uint32_t) + 1;
  char *buf = (char*)malloc(len);
  memcpy(buf, a.data(), a.size());
  char* temp = buf + a.size();
  *(uint32_t*)temp = htonl(LTS);
